   ...
   if (lister.includes(feature)) { 
     if (feature == 'buat') { 
       try { 
         ...
         conn.dropmail[id] = [ 
           await m.reply('*EMAIL:*\n' + eml[0] + '\n\n' + '◦ *ID:*\n' + eml[1] + '\n\n◦ *Kedaluwarsa:*\n' + msToTime(timeDiff) + '\n\n⌕_Contoh *' + usedPrefix + command + ' pesan* Untuk memeriksa kotak masuk Anda_'), 
           eml[0], 
           eml[1], 
           eml[2], 
         ]; 
     ...
     if (feature == 'pesan') { 
       if (!conn.dropmail[id]) return m.reply('Tidak ada pesan, harap buat email terlebih dahulu\nContoh *' + usedPrefix + command + ' buat*'); 
...
◦ *Pesan* : ${v.text} 
◦ *Ukuran* : ${formatSize(v.rawSize)} 
◦ *Subjek* : ${v.headerSubject} 
◦ *Unduhan* : ${v.downloadUrl} 
    `.trim(); 
         }).filter((v) => v).join('\n\n________________________\n\n'); 
         await m.reply(teks || '*KOSONG*' + '\n\n⌕_Contoh *' + usedPrefix + command + ' hapus* Untuk menghapus email_'); 
       } catch (e) { 
         await m.reply(eror); 
       } 
     } 
     if (feature == 'hapus') { 
       if (!conn.dropmail[id]) return m.reply('Tidak ada email yang valid'); 
